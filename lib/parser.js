...
export function parseComposition(text) {
    ...
    // ==========================================================
    const matLine = text.match(/Custo de Materiais[^|]*\|[^|]*\|\s*(R\$[^|\n]+)/i);
    if (matLine) result.mat = extractValue(matLine[1]);

    const moLine = text.match(/Custo de M[aã]o[- ]de[- ]Obra[^|]*\|[^|]*\|\s*(R\$[^|\n]+)/i);
    if (moLine) result.mo = extractValue(moLine[1]);

    const eqLine = text.match(/Custo de Equipamentos?[^|]*\|[^|]*\|\s*(R\$[^|\n]+)/i);
...
export function parseMultipleCompositions(text) {
    ...
    // - "SEÇÃO 1" or "Seção 1" (section 1 header) 
    // - **CÓDIGO:** at start of line (header field)
    // We look for either pattern preceded by a blank line or start of text
    const splitRegex = /(?=(?:^|\n)(?:\s*#{1,4}\s*)?(?:\*\*\s*)?SE[ÇC][AÃ]O\s+1[\s:—–-])/gi;
