    ...
    return (
        <div>
            ...
            {tc.hp > 0 && (
                <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <span style={{ fontWeight: 700, color: COLORS.blue, fontFamily: FONTS.mono, fontSize: 15 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                ...
                <button onClick={addG} style={{ padding: "6px 12px", borderRadius: 5, border: "none", background: COLORS.blue, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>+ Grupo</button>
            </div>
                ...
                <div style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 10 }}>
                        {aiCenarios.map((c, idx) => (
                            ...
                            >
                                <div style={{ fontSize: 16, fontWeight: 700, color: COLORS.accent, marginBottom: 6 }}>
                                    {c.nome}
                                </div>
                                {c.descricao && (
                                ...
                                {c.equipes && c.equipes.map((eq, eIdx) => (
                                    <div key={eIdx} style={{ marginBottom: 6 }}>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: COLORS.blue, marginBottom: 3 }}>{eq.grupo}</div>
                                        {eq.linhas && eq.linhas.map((l, lIdx) => (
                                            <div key={lIdx} style={{ fontSize: 13, color: COLORS.textDim, paddingLeft: 8 }}>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ fontSize: 14 }}>
                                        ...
                                        <span style={{ color: COLORS.textMuted, marginLeft: 8, fontSize: 13 }}>
                                            {c.prazo_dias || "?"} dias
                                        </span>
                                    </div>
                                    <button
            ...
            {equipes.map((g) => (
                <div key={g.id} style={{ marginBottom: 12, border: `1px solid ${COLORS.border}`, borderRadius: 6, overflow: "hidden" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", background: "#1A1710" }}>
                        ...
                        <span onClick={() => delG(g.id)} style={{ cursor: "pointer", color: COLORS.textMuted, fontSize: 13 }}>✕ Grupo</span>
                    </div>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                            <tr>
                                {["Função", "Diária R$", "Pessoas", "Dias", "Total", ""].map((h, i) => (
                                    <th key={i} style={{ padding: "4px 5px", textAlign: "left", fontSize: 12, fontWeight: 700, color: COLORS.accent, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, textTransform: "uppercase" }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {g.l.map((f, fi) => (
                                <tr key={fi}>
                                    ...
                                    <td style={{ padding: "3px 5px", borderBottom: `1px solid ${COLORS.border}`, textAlign: "right", fontFamily: FONTS.mono, color: COLORS.accent2, fontSize: 14 }}>{formatCurrency(f.d * f.p * f.di)}</td>
                                    <td style={{ padding: "3px 5px", borderBottom: `1px solid ${COLORS.border}` }}><span onClick={() => delF(g.id, fi)} style={{ cursor: "pointer", color: COLORS.textMuted, fontSize: 12 }}>✕</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <div style={{ padding: "4px 8px" }}>
                        <button onClick={() => addF(g.id)} style={{ padding: "3px 10px", borderRadius: 4, border: `1px solid ${COLORS.border}`, background: "transparent", color: COLORS.textDim, fontSize: 13, cursor: "pointer" }}>+ Função</button>
                    </div>
                </div>
            ))}

            {/* Outros Custos */}
            <h3 style={{ fontSize: 16, fontWeight: 700, margin: "16px 0 6px", fontFamily: FONTS.mono, color: COLORS.purple }}>Outros Custos de M.O.</h3>
            <div style={{ border: `1px solid ${COLORS.border}`, borderRadius: 6, marginBottom: 8 }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr>
                            {["Descrição", "Un", "Qtd", "Valor Un", "Total", ""].map((h, i) => (
                                <th key={i} style={{ padding: "4px 5px", textAlign: "left", fontSize: 12, fontWeight: 700, color: COLORS.purple, background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, textTransform: "uppercase" }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {oc.map((o) => (
                            <tr key={o.id}>
                                ...
                                <td style={{ padding: "3px 5px", borderBottom: `1px solid ${COLORS.border}`, textAlign: "right", fontFamily: FONTS.mono, color: COLORS.accent2, fontSize: 14 }}>{formatCurrency(o.q * o.v)}</td>
                                <td style={{ padding: "3px 5px", borderBottom: `1px solid ${COLORS.border}` }}><span onClick={() => dOC(o.id)} style={{ cursor: "pointer", color: COLORS.textMuted, fontSize: 12 }}>✕</span></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            </div>
            <button onClick={addOC} style={{ padding: "5px 12px", borderRadius: 5, border: `1px solid ${COLORS.border}`, background: "transparent", color: COLORS.textDim, fontSize: 14, cursor: "pointer" }}>+ Custo</button>
        </div>
    );
}
